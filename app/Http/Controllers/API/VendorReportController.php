...
class VendorReportController extends Controller
{
    ...
    public function sales(Request $request)
    {
        ...
        if ($vendorType == "service" || $vendorType == "booking") {
            $query = OrderService::select('*', DB::raw('SUM(price) as total_amount'), DB::raw('SUM(hours) as total_unit'))
            ...
            $query = Order::select('*', DB::raw('SUM(total) as total_amount'), DB::raw('COUNT(*) as total_unit'))
                ->currentStatus('delivered')
                ->where('vendor_id', $vendorId)
                ->groupBy('package_type_id');
        } else {
            $query = OrderProduct::select('*', DB::raw('SUM(price) as total_amount'), DB::raw('SUM(quantity) as total_unit'))
                ->whereHas("order", function ($q) use ($vendorId) {
                    $q->where('vendor_id', $vendorId)->currentStatus('delivered');
                })
                ->groupBy('product_id');
