...
<div>
    ...
    <div x-data="{ open: @entangle('showCreate') }">
        <x-modal confirmText="{{ __('Save') }}" action="save">
            ...
            <div class="p-2 my-2 border border-red-500 border-dashed rounded">
                <p><span class="font-bold text-red-500">{{ __('Note') }}:</span>
                    {{ __('In order of arrangement') }}.
                </p>
            ...
            <livewire:component.autocomplete-input title="{{ __('Vendor') }}" column="name" model="Vendor"
                :queryClause="$vendorSearchClause" emitFunction="autocompleteVendorSelected"
    ...
    <div x-data="{ open: @entangle('showEdit') }">
        <x-modal confirmText="{{ __('Update') }}" action="update">
            ...
            {{-- vendors --}}
            <livewire:component.autocomplete-input title="{{ __('Vendor') }}" column="name" model="Vendor"
                initialEmit="preselectedVendorEmit" emitFunction="autocompleteVendorSelected"
                onclearCalled="clearAutocompleteFieldsEvent" />
            {{-- category --}}
            <x-select title="{{ __('Category') }}" :options='$categories' name="category_id"
                selected="{{ !empty($selectedModel) ? $selectedModel->category_id : '' }}" :defer="true"
                :noPreSelect="true" />
            <x-checkbox title="{{ __('Active') }}" name="isActive" />
            <x-checkbox title="{{ __('Featured') }}"
                description="{{ __('Can featured on home screen of customer app') }}" name="featured" />
